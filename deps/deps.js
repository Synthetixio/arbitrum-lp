...
async function run() {
  ...
  const workspaceDeps = workspacePackages.map(({ name }) => [
    name,
    "workspace:*",
  ]);
  ...
  const deps = Object.fromEntries(
    [].concat(existingDeps).concat(workspaceDeps),
  );
  ...
  await workspacePackages.reduce(async (promise, { location, name }) => {
    ...
    const packageJson = JSON.parse(
      await fs.readFile(`${location}/package.json`, "utf-8"),
    );
    ...
    if (dependencies.length || missingDeps.length) {
      ...
      dependencies.sort().forEach((dep) => {
        console.log(
          `${fgRed}    "${dep}": "${packageJson.dependencies[dep]}"${fgReset}`,
        );
        delete packageJson.dependencies[dep];
      });
      missingDeps.sort().forEach((dep) => {
        console.log(
          `${fgGreen}    "${dep}": "${deps[dep] || "latest"}"${fgReset}`,
        );
    ...
    if (devDependencies.length) {
      ...
      devDependencies.sort().forEach((dep) => {
        console.log(
          `${fgRed}    "${dep}": "${packageJson.devDependencies[dep]}"${fgReset}`,
        );
  ...
  if (updatedPackages > 0 && isFix) {
    ...
    console.log("");
    console.log(
      `${fgGreen}Packages fixed: ${fgGreen}${updatedPackages}${fgReset}`,
    );
  ...
  if (updatedPackages > 0) {
    ...
    console.log("");
    console.log(
      `${fgRed}Packages need fixing: ${fgGreen}${updatedPackages}${fgReset}`,
    );
